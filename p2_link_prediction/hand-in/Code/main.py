...
def main():
    # Load the data
    edges, g_full, non_edges, _, edges_test = load_network("../Data/trainingset.csv", "../Data/testset.csv")

    # Split the data
    g_baseline, _, _, X_val, Y_val = split_dataset(edges, g_full, non_edges)
 
    # Accuracy on validation set for method 1
    scores = method_1(g_baseline, X_val)
    display_accuracy(scores, top_k=len(edges_test), Y=Y_val)

    # Predictions on test set for method 1
    scores = method_1(g_full, edges_test)
    display_test_results(scores, top_k=len(edges_test))
